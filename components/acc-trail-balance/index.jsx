...
export default function TrialBalance() {
  ...
  const getDebitTotalByLedger = (ledgerId) => {
    ...
    return journals.reduce((total, entry) => {
      if (
        entry.field_debit_account.id === ledgerId
      ) {
        return (
          total +
          Number(
            entry.field_amount || 0
          )
        );
      }
      return total;
  ...
  const getCreditTotalByLedger = (ledgerId) => {
    ...
    return journals.reduce((total, entry) => {
      if (
        entry.field_credit_account.id === ledgerId
      ) {
        return (
          total +
          Number(
  ...
  useEffect(() => {
    ...
    const debitTotal = journals.reduce(
      (sum, entry) =>
        sum +
        Number(
          entry.field_amount || 0
    ...
    const creditTotal = journals.reduce(
      (sum, entry) =>
        sum +
        Number(
          entry.field_amount || 0
